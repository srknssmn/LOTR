import { GOLDTOKEN_ADDRESS, LOTR_ADDRESS } from "/constants/address.js";
...
export const approve = async () => {
    ...
    const _contract = await new ethers.Contract(GOLDTOKEN_ADDRESS, ERC20, signer);
    // Max. token kullanım izni veriyoruz!
    const txn = await _contract.approve(LOTR_ADDRESS, ethers.constants.MaxUint256);
...
export const getAllowance = async () => {
    const provider = new ethers.providers.Web3Provider(window.ethereum);
    const signer = provider.getSigner();
    const _contract = new ethers.Contract(GOLDTOKEN_ADDRESS, ERC20, provider);
    const result = await _contract.allowance(signer.getAddress(), LOTR_ADDRESS);
    return result;
}